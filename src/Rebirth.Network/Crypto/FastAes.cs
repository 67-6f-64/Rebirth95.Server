	...
	public sealed class FastAes
	{
		...
		internal unsafe void TransformBlock(byte[] block)
		{
			fixed (byte* pBlock = block)
			{
				...
				while (curRound < (Rounds - 0x2))
				{
					wk = WorkingKey[++curRound];
					r0 = Table0[C0 & 0xFF] ^ Table1[(C1 >> 0x8) & 0xFF] ^ Table2[(C2 >> 0x10) & 0xFF] ^ Table3[C3 >> 0x18] ^ wk[0x0];
					r1 = Table0[C1 & 0xFF] ^ Table1[(C2 >> 0x8) & 0xFF] ^ Table2[(C3 >> 0x10) & 0xFF] ^ Table3[C0 >> 0x18] ^ wk[0x1];
					r2 = Table0[C2 & 0xFF] ^ Table1[(C3 >> 0x8) & 0xFF] ^ Table2[(C0 >> 0x10) & 0xFF] ^ Table3[C1 >> 0x18] ^ wk[0x2];
					r3 = Table0[C3 & 0xFF] ^ Table1[(C0 >> 0x8) & 0xFF] ^ Table2[(C1 >> 0x10) & 0xFF] ^ Table3[C2 >> 0x18] ^ wk[0x3];
					wk = WorkingKey[++curRound];
					C0 = Table0[r0 & 0xFF] ^ Table1[(r1 >> 0x8) & 0xFF] ^ Table2[(r2 >> 0x10) & 0xFF] ^ Table3[r3 >> 0x18] ^ wk[0x0];
					C1 = Table0[r1 & 0xFF] ^ Table1[(r2 >> 0x8) & 0xFF] ^ Table2[(r3 >> 0x10) & 0xFF] ^ Table3[r0 >> 0x18] ^ wk[0x1];
					C2 = Table0[r2 & 0xFF] ^ Table1[(r3 >> 0x8) & 0xFF] ^ Table2[(r0 >> 0x10) & 0xFF] ^ Table3[r1 >> 0x18] ^ wk[0x2];
					C3 = Table0[r3 & 0xFF] ^ Table1[(r0 >> 0x8) & 0xFF] ^ Table2[(r1 >> 0x10) & 0xFF] ^ Table3[r2 >> 0x18] ^ wk[0x3];
				}
				...
				wk = WorkingKey[++curRound];
				C0 = (uint)Sbox[r0 & 0xFF] ^ (((uint)Sbox[(r1 >> 0x8) & 0xFF]) << 0x8) ^ (((uint)Sbox[(r2 >> 0x10) & 0xFF]) << 0x10) ^ (((uint)Sbox[r3 >> 0x18]) << 0x18) ^ wk[0x0];
				C1 = (uint)Sbox[r1 & 0xFF] ^ (((uint)Sbox[(r2 >> 0x8) & 0xFF]) << 0x8) ^ (((uint)Sbox[(r3 >> 0x10) & 0xFF]) << 0x10) ^ (((uint)Sbox[r0 >> 0x18]) << 0x18) ^ wk[0x1];
				C2 = (uint)Sbox[r2 & 0xFF] ^ (((uint)Sbox[(r3 >> 0x8) & 0xFF]) << 0x8) ^ (((uint)Sbox[(r0 >> 0x10) & 0xFF]) << 0x10) ^ (((uint)Sbox[r1 >> 0x18]) << 0x18) ^ wk[0x2];
				C3 = (uint)Sbox[r3 & 0xFF] ^ (((uint)Sbox[(r0 >> 0x8) & 0xFF]) << 0x8) ^ (((uint)Sbox[(r1 >> 0x10) & 0xFF]) << 0x10) ^ (((uint)Sbox[r2 >> 0x18]) << 0x18) ^ wk[0x3];
		...
		{
			int KeyLength = key.Length / 0x4;  // key length in words

			uint[][] KeyResult = new uint[Rounds + 0x1][]; // 4 words in a block
			for (int i = 0x0; i <= Rounds; ++i)
			{
				KeyResult[i] = new uint[0x4];
			}

			// copy the key into the round key array
			for (int t = 0x0, i = 0x0; i < key.Length; t++)
			{
				KeyResult[t >> 0x2][t & 0x3] = LeToUInt32(key, i);
				i += 0x4;
			}

			// while not enough round key material calculated
			// calculate new values
			int k = (Rounds + 0x1) << 0x2;
			for (int i = KeyLength; (i < k); i++)
			{
				uint temp = KeyResult[(i - 0x1) >> 0x2][(i - 0x1) & 0x3];
				if ((i % KeyLength) == 0x0)
				{
					temp = SubWord(Shift(temp, 0x8)) ^ Rcon[(i / KeyLength) - 0x1];
				}
				else if ((KeyLength > 0x6) && ((i % KeyLength) == 0x4))
				{
					temp = SubWord(temp);
				}

				KeyResult[i >> 0x2][i & 0x3] = KeyResult[(i - KeyLength) >> 0x2][(i - KeyLength) & 0x3] ^ temp;
			}
		...
		{
			return unchecked((x >> shift) | (x << (0x20 - shift)));
		}

		private static uint SubWord(uint x)
		{
			return (uint)Sbox[x & 0xFF]
				| (((uint)Sbox[(x >> 0x8) & 0xFF]) << 0x8)
				| (((uint)Sbox[(x >> 0x10) & 0xFF]) << 0x10)
				| (((uint)Sbox[(x >> 0x18) & 0xFF]) << 0x18);
		}

		private static uint LeToUInt32(byte[] buffer, int off)
		{
			return (uint)buffer[off]
				| (uint)buffer[off + 0x1] << 0x8
				| (uint)buffer[off + 0x2] << 0x10
				| (uint)buffer[off + 0x3] << 0x18;
		}
		#endregion
		...
		private const byte Rounds = 0xE; //Maplestory uses a 256bit key, thus we dont need support for anything else
