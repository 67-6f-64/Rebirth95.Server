...
namespace Rebirth.Tools.FromWvsGlobal
{
	class CRC32
	{
		...
		public static uint CalcCrc32(IEnumerable<byte> b, int length, uint baseValue = 0, int bufferOffset = 0)
		{
			return b.Skip(bufferOffset)
					.Take(length)
					.Aggregate(
						   baseValue,
						   (currentChecksum, currentByte) =>
							   (currentChecksum << 8) ^ table[((currentChecksum >> 24) & 0xFF) ^ currentByte]
					   ) & 0xFFFFFFFF;
